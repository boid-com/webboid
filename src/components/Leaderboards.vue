...
div
    ...
    .col-xs-12
      q-card(v-if="globalStats")
        ...
        .row.justify-center.items-center
          ...
          .col-auto
            q-btn(@click="openURL('https://stats.boid.com')" big color="blue") More Stats
  .row
  ...
  .row
    .col-md-12.col-lg-6(style="padding-bottom: 15px")
      q-card.animate-scale.relative-position(style="height: 100%" v-if="tiersLeaderboard")
        ...
        .row.justify-between
          .col-auto
            ...
            .row
              q-btn(:class="{ tierBtn: index === selectedTier }" :key="index" @click="selectedTier = index" big v-for="(tier,index) of tiersLeaderboard.length") {{ index }}
          .col-auto
            .row.justify-center
              small.text-white 1
            div(style="padding: 4px")
            q-btn(:class="{ tierBtn: showSocial }" @click="selectedTier = null" big) social
        div(style="padding: 15px")
        table.q-table.horizontal-separator(style="width: 100%" v-if="!showSocial")
          thead
            tr
              th
              th.relative-position(style="width: 50px")
          ...
          tbody.full-width(:key="user.id" v-for="(user, index) in selectedLeaderboard")
            tr.user(@click="$router.push({ name: 'User', params: { username: user.username } })" style="cursor: pointer")
              td.relative-position
                .absolute-left.text-grey-7
                  .puck.text-center {{ index + 1 }}
                .absolute-left.text-grey-7(style="left: 45px; top: 45px")
                ...
                .row
                  ...
                  .col.relative-position(style="padding: 15px; padding-bottom: 30px")
                    .ellipsis(data-th="Username") {{ user.username }}
                    small.ellipsis.block.light-paragraph.absolute(style="bottom: 0px; max-width: 100%") {{ user.tagline }}
              td(data-th="Power" style="width: 80px") {{ parseInt(user.tPower).toLocaleString() }}
              //- td(data-th="Power") {{parseInt(user.tPower).toLocaleString()}}
        table.q-table.horizontal-separator(style="width: 100%" v-else)
          thead
            tr
              ...
              th.relative-position(style="width: 30px")
                ...
                q-icon.absolute-center(color="grey-8" name="fa-users" style="top: 15px")
              th.relative-position(style="width: 30px")
          ...
          tbody.full-width(:key="user.id" v-for="(user, index) in socialLeaderboard")
            tr.user(@click="$router.push({ name: 'User', params: { username: user.username } })" style="cursor: pointer")
              td.relative-position
                .absolute-left.text-grey-7
                  .puck.text-center {{ index + 1 }}
    ...
    .col-md-12.col-lg-6
      q-card.animate-scale
        ...
        table.q-table.horizontal-separator(style="width: 100%")
          ...
          tbody(:key="team.id" v-for="(team, index) in teamLeaderboard")
