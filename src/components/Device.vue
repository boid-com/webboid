...
div
  .layout-padding.relative-position(v-if="!loading && thisDevice.name")
    h6.text-center(style="padding: 0px; margin: 0px") {{ thisDevice.name }}
      q-icon.float-lefth.on-right.inline(:color="parseDevice.color(thisDevice)" :name="parseDevice.icon(thisDevice)" style="padding-bottom: 3px")
      q-tooltip Your Device Name

    q-card(style="width: 90vw; max-height: 410px")
      q-card-media.relative-position
        q-btn.infobtn.absolute-top-right(@click="openConfigModal()" color="blue" flat round)
          q-icon(color="grey-7" name="settings")
          q-tooltip Settings
        .row
          .col-auto
            img(src="statics/images/magnifyingglass.svg" style="opacity: 0.9; width: 100px; height: 100px; padding: 15px")
          .col-7.relative-position
            ...
            .relative-position(style="width: 70%")
              ...
              div(style="padding-left: 20px; padding-top: 5px")
                ...
                q-btn.absolute-right(@click="refreshDevice()" flat)
                  q-tooltip Refresh Device
                  q-icon(name="refresh")
            .row.justify-center
              q-btn.light-paragraph(@click="ipcRenderer.send('openURL', 'https://www.worldcommunitygrid.org/research/mcm1/overview.do')" flat style="margin-bottom: 5px") Mapping Cancer Markers
      ...
      q-card-main(style="max-height: 195px; overflow: scroll; padding-top: 0px" v-if="toggle")
        div(:key="task.slot[0]" style="margin-bottom: 5px" v-for="(task, index) in activeTasks" v-if="activeTasks.length > 0")
          q-progress(:buffer="0" :percentage="modulateTaskProgress(task.checkpoint_fraction_done[0])" height="40px" stripe style="height: 10px" v-if="task.active_task_state[0] == 1 && !onBatteries")
          q-progress(:buffer="0" :percentage="task.checkpoint_fraction_done[0] * 100" color="grey-4" height="40px" stripe v-else)
          q-tooltip
      ...
      q-card-actions.taller.relative-position
        q-btn(flat round small)
          q-icon.on-right(:name="boincStatusIcon" v-if="toggle")
        h6.text-grey-8.on-right(style="padding-top: 5px" v-if="toggle") {{ boincStatus }}
        q-spinner-grid.inline.on-right.absolute-right(:size="20" color="grey-4" style="right: 70px; top: 20px" v-if="toggle")
  ...
  .layout-padding.relative-position(v-else)
    ...
    q-btn.absolute-center(@click="refreshDevice()" big flat)
      q-tooltip Refresh Device
      q-icon(name="refresh")
    .row.justify-center
      q-btn(@click="ipcRenderer.send('openURL', 'https://www.youtube.com/watch?v=VVlGjVDek_M')" flat) Problems?
</template>
...
export default {
  ...
  methods: {
    ...
    modulateTaskProgress(progress) {
      ...
      if (progress < 0.02) progress += 0.01
